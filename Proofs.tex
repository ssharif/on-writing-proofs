...
\end{quote}
is true or not. We have P is ``the sky is green'' and Q is ``coffee is the best beverage in the universe''. Since P is false and Q is true, we're in the third row of the table. The last entry is T, so the statement is true.
...
\begin{itemize}
    ...
    \item The set of all example sets in this section, including this one.
\end{itemize}
...
The cardinality of $\{1, 2, 3\}$ is 3. If $S = \{2, 4, 6, \dots, 20\}$, then $\# S = 10$. If $S = \emptyset$, then $\# S = 0$. Note that the cardinality of a finite set is always a nonnegative integer. \marginnote{There are such things as infinite cardinalities; we will discuss some basic concepts in the theory later in this course.}
...
In mathematics, we use the term \emph{rigor} to mean the practice of not skipping any steps, of covering every part and detail in an argument. To become a rigorous writer takes practice, both as a \emph{writer} and as a \emph{reader} of mathematics. But rigor is most important at the end of the problem-solving process. One way of thinking of this is that when you solve a problem, you should prove it three times:\sidenote{Thanks to G.~Harel for this idea.}
\begin{itemize}
    \item once for yourself,
    \item once for your friend,
    \item lastly for your enemy.
\end{itemize}
