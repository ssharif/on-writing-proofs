...
\end{itemize}
  ...
  \begin{quote}
    For every integer $n$, $n$ is prime.\sidenote{To make the sentence grammatically correct, I've moved the word ``integer''.}
  \end{quote}
...
A set is essentially\sidenote{Set theory is actually quite complicated, and in certain situations you have to be very careful about what is considered a set. However in most mathematical circumstances, this is a rare occurence.} a collection of things. These things can be numbers, functions, people, symbols, geometric objects, or even other sets. 
...
There's a major problem with this representation of sets: if there are a lot, even infinitely many elements of the set, it can be hard to write them all down! Fortunately, we can use the magic of the ellipses\sidenote{Better known as ``dot dot dot''.}:
\begin{itemize}
...
\end{itemize}
Essentially, the $\dots$ says, ``You know what I mean'', or if you prefer, ``Yadda yadda yadda''.\marginnote{Little-known fact: the ancient Greek referred to a dot as a \emph{yadda}.}
