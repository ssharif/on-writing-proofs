...
\begin{itemize}
    \item ``If he is a baby, then she cannot drive a car'' vs.\ ``If he cannot drive a car, then he is a baby.''
...
\begin{quote}
  For all integers $x$, if $x$ is prime, then $x$ is odd.
\end{quote}
The first sentence \emph{could be} viewed as true if, say, we knew that $x = 3$. However, since the ``for all'' part is understood, the statement is false, since $x = 2$ is a prime which is not odd.
...
\begin{quote}
  $\exists x$ such that some condition holds. \marginnote{``There exists'' always pairs with ``such that''.}
\end{quote}
The statement is true if there is \emph{at least one} $x$ for which the condition holds. For example,
\begin{itemize}
...
When negating quantified statements, the quantifiers ``swap''.\marginnote{I highly recommend spending five minutes with each of these pairs of statements to understand why one is the negation of the other. Don't try to understand these mechanically; really try to understand what it would mean for the first sentence to be false.}
\begin{itemize}
...
\]
The vertical bar is read ``such that''.\marginnote{Some texts use a colon instead of a vertical bar.} Some examples:
\begin{enumerate}
...
The second set is all integer multiples of $4$ (why?), and the last set is the interval $(1, \infty)$. This last is the set of squares of all numbers smaller than $-1$; for example, $(-1.5)^2$, $(-2)^2$, $(-10)^2$, and so forth all lie in this set.
